...
def main() -> None:
    ...
    if not out_path.parent.exists():
        try:
            ...
            raise RuntimeError(f"出力ディレクトリを作成できません: {exc}") from exc
    ...
    for i, w in enumerate(wav_list):
        ...
        if i != len(wav_list) - 1:
            out_wav.append(sil)
    merged = np.concatenate(out_wav, axis=0) if out_wav else np.zeros((0,), dtype=np.float32)
...
if __name__ == "__main__":
    try:
        ...
        logging.getLogger("qwen_tts").exception("VoiceCloneError: %s", err)
        sys.exit(1)
    except Exception:
